...
export default function RenderFollowers(){
    ...
    useEffect(()=>{
        const fetchData = async () => {
            try {
                let res= await retrieveFollowers(username)
                setData(res);
            } catch (error) {
    ...
    return(<>
        ...
        {data.length>0 && data.map((follow,index)=>{
            return(
                <div key={index} className="follow" onClick={()=>{navigate(`/Profile/${follow.sendingRelation.username}`)}}>
                    <RenderPostAvatar url={follow.sendingRelation.profilePic}/>
                    <p style={{width:"50%",
                        textOverflow: "ellipsis",textAlign:"start",
                        whiteSpace: "nowrap",
                        overflow: "hidden"}} >{follow.sendingRelation.username}</p>
